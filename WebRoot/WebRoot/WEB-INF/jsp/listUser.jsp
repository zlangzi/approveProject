	...
	<head>
		...
		<script type="text/javascript">
			...
			function getResponseJson(){
				var param = "name="+encodeURI($('#name').val())+"&age="+$('#age').val()+"&phone="+$('#phone').val()+"&page=1&pageCount=6";
				var url = "ajaxGetUser.?"+param;
				//alert(param);
			...
			function createTbody(json){
				...
				$.each(json,function(i, n){
					if(i%2==0){
						tableBody.append('<tr class="gradeX2"></tr>'); 
					}else{
						tableBody.append('<tr class="gradeC2"></tr>'); 
					}
					...
					tr.append("<td><a href=\"${pageContext.request.contextPath}/toUpdateUser.?uid="+uid+"\">更新</a>|"+
			...
			<c:choose>
				...
				<c:otherwise>
					<table cellpadding="5" cellspacing="0" border="1" class="display"
						id="myTable" align="center" style="background: #fff">
						<thead>
							<tr align="center">
								<th></th>
						...
						<tbody align="center">
							<c:forEach var="user" items="${users}" varStatus="count">
								<c:choose>
									...
									<td>${user.phone}</td>
									<td>
										<a href="${pageContext.request.contextPath}/toUpdateUser.action?uid=${user.id}">更新</a>|
										<a href="${pageContext.request.contextPath}/deleteUser.action?uid=${user.id}">删除</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
